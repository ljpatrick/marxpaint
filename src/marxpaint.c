...
/* Color depth for Marx Paint to run in, and store canvases in: */
...
/* workaround dirent handling bug in MarxPaint code */
...
/*
 ...
 SDL_Pango.h is included by marxpaint.c.
 */
...
static SDL_Rect r_marxarea;      /* was 640x56 */
...
/**
 * Sets button_scale to the maximum Marx Paint can handle
 */
static void set_max_buttonscale(void)
{
  ...
  /* WINDOW_HEIGHT / original size of r_ttools.h + 5 buttons + colors rows + marx area */
 ...
 * size of the window/screen Marx Paint is being displayed on
...
static void setup_normal_screen_layout(void)
{
  ...
  r_marxarea.x = 0;
  r_marxarea.w = WINDOW_WIDTH;

  /* need 56 minimum for the Marx area */
  ...
  r_marxarea.y = r_colors.y + r_colors.h;
  r_marxarea.h = WINDOW_HEIGHT - r_marxarea.y;

  r_sfx.x = r_marxarea.x;
  r_sfx.y = r_marxarea.y;
...
/**
 * Debug output that shows Marx Paint's layout
 */
 ...
 * position, size, and layout of Marx Paint's UI
 */
...
static SDL_Surface *img_title, *img_title_credits, *img_title_marxpaint;
 ...
 */
static SDL_Surface *render_text(MarxPaint_Font * restrict font, const char *restrict str, SDL_Color color)
{
 ...
 * @return A new surface, containing the rendered text
 */
static SDL_Surface *render_text_w(MarxPaint_Font * restrict font, const wchar_t * restrict str, SDL_Color color)
{
  SDL_Surface *ret = NULL;
  int height;
  Uint16 *ustr;
...
static SDL_Surface *img_marx[NUM_TIP_TUX];
...
static void redraw_marx_text(void);
static void draw_marx_text(int which_marx, const char *const str, int want_right_to_left);
static void draw_marx_text_ex(int which_marx, const char *const str, int want_right_to_left, Uint8 locale_text);
...
/* FIXME: debug junk */
              ...
              if (key == SDLK_ESCAPE && !disable_quit)
                      ...
                      if (mute)
                        {
                          ...
                          draw_marx_text(TUX_BORED, gettext("Sound muted."), 0);
                        }
                      else
                        {
                          /* Sound has been unmuted (unsilenced) via keyboard shortcut */
                          draw_marx_text(TUX_BORED, gettext("Sound unmuted."), 0);
                        }
                    }
#endif
                }
              ...
              else if (key == SDLK_o && (mod & KMOD_CTRL) && !noshortcuts)
                {
                  ...
                  draw_marx_text(TUX_GREAT, tool_tips[cur_tool], 1);
              ...
              else if ((key == SDLK_n && (mod & KMOD_CTRL)) && !noshortcuts)
                {
                  ...
                  if (do_new_dialog() == 0)
                    {
                      draw_marx_text(tool_marx[TUX_DEFAULT], TIP_NEW_ABORT, 1);
              ...
              else
                {
                  ...
                  if (((cur_tool == TOOL_TEXT || cur_tool == TOOL_LABEL) && cursor_x != -1 && cursor_y != -1) ||
                      (cur_tool == TOOL_LABEL && cur_label == LABEL_SELECT))
                    {
                      ...
                      while (*im_cp)
                        {
                          if (*im_cp == L'\b')
                          ...
                          else if (*im_cp == L'\r')
                            {
                              ...
                              font_height = MarxPaint_Font_FontHeight(getfonthandle(cur_font));
                                      ...
                                      for (j = 0; j < num_font_families; j++)
                                        {
                                          if (user_font_families[j] && user_font_families[j]->handle)
                                            {
                                              MarxPaint_Font_CloseFont(user_font_families[j]->handle);
                      ...
                      if (im_data.tip_text)
                        {
                          draw_marx_text(TUX_DEFAULT, im_data.tip_text, 1);
                        }
                      ...
                      if (whicht < NUM_TOOLS && tool_avail[whicht] &&
                          (valid_click(event.button.button) || whicht == TOOL_PRINT))
                        {
                          ...
                          if (cur_tool != TOOL_TEXT)
                            draw_marx_text(tool_marx[cur_tool], tool_tips[cur_tool], 1);
                              ...
                              if (!font_thread_done)
                                {
                                  ...
                                  draw_marx_text(TUX_WAIT, gettext("Please wait…"), 1);
                      ...
                      else if (HIT(r_controls))
                        {
                          ...
                          if (cur_tool == TOOL_STAMP)
                          ...
                          else if (cur_tool == TOOL_SHAPES)
                            {
                              ...
                              draw_shapes();
			      update_screen_rect(&r_toolopt);
                              draw_marx_text(TUX_GREAT, shapemode_tips[shape_mode], 1);
                              playsound(screen, 0, SND_CLICK, 0, SNDPOS_RIGHT, SNDDIST_NEAR);
                              update_screen_rect(&r_marxarea);
                              toolopt_changed = 0;
                            }
                          else if (cur_tool == TOOL_TEXT)
                            {
                                      ...
                                      for (i = 0; i < num_font_families; i++)
                                        {
                                          if (user_font_families[i] && user_font_families[i]->handle)
                                            {
                                              MarxPaint_Font_CloseFont(user_font_families[i]->handle);
                      ...
                      if (cur_tool == TOOL_BRUSH || cur_tool == TOOL_LINES)
                        ...
                        {
                          cur_fill = cur_thing;
                          draw_marx_text(TUX_GREAT, fill_tips[cur_fill], 1);
                        ...
                        {
                          /* FIXME */
                          /* char font_marx_text[512]; */

                          cur_font = cur_thing;

                          /* FIXME */
                          /*
                             safe_snprintf(font_marx_text, sizeof font_marx_text, "%s (%s).",
                             TTF_FontFaceFamilyName(getfonthandle(cur_font)),
                             TTF_FontFaceStyleName(getfonthandle(cur_font)));
                             draw_marx_text(TUX_GREAT, font_marx_text, 1);
                           */
                          ...
                          if (stamp_data[stamp_group][cur_stamp[stamp_group]]->stxt != NULL)
                            {
                              ...
                              draw_marx_text_ex(TUX_GREAT, stamp_data[stamp_group][cur_stamp[stamp_group]]->stxt, 1,
                                               stamp_data[stamp_group][cur_stamp[stamp_group]]->locale_text);
                            }
                          else
                            draw_marx_text(TUX_GREAT, "", 0);
                      ...
                      else if (cur_tool == TOOL_SHAPES)
                        {
                          ...
                          if (toolopt_changed)
                            draw_marx_text(TUX_GREAT, shape_tips[cur_shape], 1);
                      ...
                      else if (cur_tool == TOOL_MAGIC)
                        {
                          ...
                          draw_marx_text(TUX_GREAT, magics[cur_magic].tip[magic_modeint(magics[cur_magic].mode)], 1);
                  ...
                  if (valid_click(event.button.button))
                    {
                      ...
                      if (whichc >= 0 && whichc < NUM_COLORS)
                        {
                          cur_color = whichc;
                          draw_marx_text(TUX_KISS, color_names[cur_color], 1);
                      ...
                      if (!emulate_button_pressed)
                        {
                          ...
                          draw_marx_text(TUX_BORED, TIP_LINE_START, 1);
                        }
                      if (mouseaccessibility)
                        emulate_button_pressed = !emulate_button_pressed;
                    }
                      ...
                      if (shape_tool_mode == SHAPE_TOOL_MODE_DONE)
                        {
                          ...
                          playsound(screen, 1, SND_LINE_START, 1, event.button.x, SNDDIST_NEAR);
                          draw_marx_text(TUX_BORED, TIP_SHAPE_START, 1);
                          if (mouseaccessibility)
                          ...
                          if (mouseaccessibility)
                            {
                              ...
                              shape_tool_mode = SHAPE_TOOL_MODE_DONE;
                              draw_marx_text(TUX_GREAT, tool_tips[TOOL_SHAPES], 1);
                            }
                        }
                  ...
                  if (event.user.data1 != NULL)
                    {
                      if (((unsigned char *)event.user.data1)[0] == '=')
                        {
                          draw_marx_text_ex(TUX_GREAT, (char *)event.user.data1 + 1, 1, (int)(intptr_t) event.user.data2);       //EP added (intptr_t) to avoid warning on x64
                        }
                      else
                        {
                          draw_marx_text_ex(TUX_GREAT, (char *)event.user.data1, 0, (int)(intptr_t) event.user.data2);   //EP added (intptr_t) to avoid warning on x64
                        }
                    }
                  else
                    draw_marx_text(TUX_GREAT, "", 1);
                }
                      ...
                      if (old_x >= 0 && old_y >= 0 && old_x <= r_canvas.w && old_y <= r_canvas.h)
                        {
                          ...
                          draw_marx_text(TUX_GREAT, great_str(), 1);
                      ...
                      if (!mouseaccessibility || (mouseaccessibility && !emulate_button_pressed))
                        {
                          ...
                          playsound(screen, 1, SND_LINE_END, 1, event.button.x, SNDDIST_NEAR);
                          draw_marx_text(TUX_GREAT, tool_tips[TOOL_LINES], 1);
                        }
                    }
                              ...
                              if (!simple_shapes && !shape_no_rotate[cur_shape])
                                {
                                  ...
                                  playsound(screen, 1, SND_LINE_START, 1, event.button.x, SNDDIST_NEAR);
                                  draw_marx_text(TUX_BORED, TIP_SHAPE_NEXT, 1);
...
static void draw_blinking_cursor(void)
{
  ...
  line_xor(cursor_x + cursor_textwidth, cursor_y,
           cursor_x + cursor_textwidth, cursor_y + MarxPaint_Font_FontHeight(getfonthandle(cur_font)));

  update_screen(cursor_x + r_canvas.x + cursor_textwidth,
                cursor_y + r_canvas.y,
                cursor_x + r_canvas.x + cursor_textwidth,
                cursor_y + r_canvas.y + MarxPaint_Font_FontHeight(getfonthandle(cur_font)));
}
 ...
 *
 * The original Marx Paint canvas was 448x376. The canvas can be
...
static void loadstamp_finisher(stamp_type * sd, unsigned w, unsigned h, double ratio)
{
  ...
  /* If Marx Paint is in mirror-image-by-default mode, mirror, if we can: */
  ...
  do
    {
      ...
      /* By default, Marx Paint allowed stamps to be, at max, 2x as wide OR 2x as tall as canvas; scaled that back to 1.5 -bjk 2011.01.08 */
...
static void load_stamps(SDL_Surface * screen)
{
  ...
  load_stamp_dir(screen, "/Library/Application Support/MarxPaint/stamps");
#endif
#ifdef __IOS__
...
static int generate_fontconfig_cache_real(void)
{
  ...
  tmp_font = MarxPaint_Font_OpenFont(PANGO_DEFAULT_FONT, NULL, 12);
  ...
  if (tmp_font != NULL)
    {
      ...
      MarxPaint_Font_CloseFont(tmp_font);
    }
  else
    {
#ifdef DEBUG
 ...
 * FIXME
 */
/* Draw marx's text on the screen: */
static void draw_marx_text(int which_marx, const char *const str, int want_right_to_left)
{
  draw_marx_text_ex(which_marx, str, want_right_to_left, 0);
}

static int latest_marx;
static const char *latest_marx_text;
...
static void redraw_marx_text(void)
{
  draw_marx_text_ex(latest_marx, latest_marx_text, latest_r2l, latest_locale_text);
}
...
static void draw_marx_text_ex(int which_marx, const char *const str, int want_right_to_left, Uint8 locale_text)
{
  ...
  latest_marx = which_marx;
  latest_marx_text = str;
  ...
  /* Clear first: */
  SDL_FillRect(screen, &r_marxarea, SDL_MapRGB(screen->format, 255, 255, 255));

  /* Draw marx: */
  dest.x = r_marxarea.x;
  dest.y = r_marxarea.y + r_marxarea.h - img_marx[which_marx]->h;

  /* if he's too tall to fit, go off the bottom (not top) edge */
  if (dest.y < r_marxarea.y)
    dest.y = r_marxarea.y;

  /* Don't let sfx and speak buttons cover the top of Marx, either: */
  if (cur_tool == TOOL_STAMP && use_sound && !mute)
    {
  ...
  SDL_BlitSurface(img_marx[which_marx], NULL, screen, &dest);

  /* Wide enough for Marx, or two stamp sound buttons (whichever's wider) */
  w = max(img_marx[which_marx]->w, img_btnsm_up->w * 2) + 5;

  wordwrap_text_ex(str, black, w, r_marxarea.y, r_marxarea.w, want_right_to_left, locale_text);
  ...
  if (cur_tool == TOOL_STAMP && use_sound && !mute)
    {
      ...
      dest.y = r_marxarea.y + ((img_btnsm_up->h - img_sfx->h) / 2);
      ...
      dest.y = r_marxarea.y + ((img_btnsm_up->h - img_speak->h) / 2);
...
{
  SDL_Surface *text;
  MarxPaint_Font *myfont = medium_font;
  SDL_Rect dest;
...
static void save_current(void)
{
  ...
  if (!make_directory(DIR_SAVE, "", "Can't create user data directory (E001)"))
    {
      draw_marx_text(TUX_OOPS, strerror(errno), 0);
      return;
    }
...
static void cleanup(void)
{
  ...
  free_surface_array(img_marx, NUM_TIP_TUX);
...
#endif
      MarxPaint_Font_CloseFont(medium_font);
      medium_font = NULL;
    }
...
#endif
      MarxPaint_Font_CloseFont(small_font);
      small_font = NULL;
    }
...
#endif
      MarxPaint_Font_CloseFont(large_font);
      large_font = NULL;
    }
  ...
  /* If we're using a lockfile, we can 'clear' it when we quit
     (so Marx Paint can be launched again soon, if the user wants to!) */
...
static int do_save(int tool, int dont_show_success_results)
{
  ...
  /* Make sure we have a ~/.marxpaint directory: */
  ...
  /* Make sure we have a ~/.marxpaint/saved directory: */
  ...
  /* Make sure we have a ~/.marxpaint/saved/.thumbs/ directory: */
  ...
  /* Make sure we have a ~/.marxpaint/saved/.label/ directory: */
  ...
  if (!dont_show_success_results)
    {
      draw_marx_text(TUX_DEFAULT, tool_tips[TOOL_SAVE], 1);
      do_setcursor(cursor_arrow);
    }
                           ...
                           size_t dataLen)
{
  ...
  safe_strncat(headers, "Marxpaint\n", headers_sz);
  safe_strncat(headers, "Marxpaint_" VER_VERSION "\n", headers_sz);
...
static void do_png_embed_data(png_structp png_ptr)
{
  ...
  /*
     ...
     The string "Marxpaint" to easy identify them as Marxpaint chunks.
     A string identifying the sofware who created it, in our case "Marxpaint_"VER_VERSION No spaces allowed
  ...
  png_unknown_chunk marxpaint_chunks[5];
...
/* clear the transparent pixels assigning the same r g and b values */
      ...
      marxpaint_chunks[1].data = (png_byte *) chunk_data;
      marxpaint_chunks[1].size = chunk_data_len;
      marxpaint_chunks[1].location = PNG_HAVE_IHDR;
      marxpaint_chunks[1].name[0] = 't';
      marxpaint_chunks[1].name[1] = 'p';
      marxpaint_chunks[1].name[2] = 'F';
      marxpaint_chunks[1].name[3] = 'G';
      marxpaint_chunks[1].name[4] = '\0';
      png_write_chunk(png_ptr, marxpaint_chunks[1].name, marxpaint_chunks[1].data, marxpaint_chunks[1].size);
  ...
  if (img_starter_bkgd)
    {
      ...
      marxpaint_chunks[2].data = (png_byte *) chunk_data;
      marxpaint_chunks[2].size = chunk_data_len;
      marxpaint_chunks[2].location = PNG_HAVE_IHDR;
      marxpaint_chunks[2].name[0] = 't';
      marxpaint_chunks[2].name[1] = 'p';
      marxpaint_chunks[2].name[2] = 'B';
      marxpaint_chunks[2].name[3] = 'G';
      marxpaint_chunks[2].name[4] = '\0';
      png_write_chunk(png_ptr, marxpaint_chunks[2].name, marxpaint_chunks[2].data, marxpaint_chunks[2].size);
  ...
  if (label && are_labels())
    {
      ...
      marxpaint_chunks[3].data = chunk_data;
      marxpaint_chunks[3].size = chunk_data_len;
      marxpaint_chunks[3].location = PNG_HAVE_IHDR;
      marxpaint_chunks[3].name[0] = 't';
      marxpaint_chunks[3].name[1] = 'p';
      marxpaint_chunks[3].name[2] = 'L';
      marxpaint_chunks[3].name[3] = 'D';
      marxpaint_chunks[3].name[4] = '\0';

      png_write_chunk(png_ptr, marxpaint_chunks[3].name, marxpaint_chunks[3].data, marxpaint_chunks[3].size);
      free(compressed_data);
      free(chunk_data);
      free(sbk_pixs);
      ...
      marxpaint_chunks[4].data = chunk_data;
      marxpaint_chunks[4].size = chunk_data_len;
      marxpaint_chunks[4].location = PNG_HAVE_IHDR;
      marxpaint_chunks[4].name[0] = 't';
      marxpaint_chunks[4].name[1] = 'p';
      marxpaint_chunks[4].name[2] = 'L';
      marxpaint_chunks[4].name[3] = 'L';
      marxpaint_chunks[4].name[4] = '\0';

      png_write_chunk(png_ptr, marxpaint_chunks[4].name, marxpaint_chunks[4].data, marxpaint_chunks[4].size);
  ...
  if (starter_id[0] != '\0' ||
      template_id[0] != '\0' || canvas_color_r != 255 || canvas_color_g != 255 || canvas_color_b != 255)
    {
      ...
      marxpaint_chunks[4].name[0] = 't';
      marxpaint_chunks[4].name[1] = 'p';
      marxpaint_chunks[4].name[2] = 'D';
      marxpaint_chunks[4].name[3] = 'T';
      marxpaint_chunks[4].name[4] = '\0';
      ...
      else
        {
          if (setjmp(png_jmpbuf(png_ptr)))
          ...
          else
            {
              ...
              text_ptr[count].text = (png_charp) "Marx Paint " VER_VERSION " (" VER_DATE ")";
              text_ptr[count].compression = PNG_TEXT_COMPRESSION_NONE;
              count++;
  ...
  do
    {
      ...
      if (num_files == 0)
      ...
      else
        {
          ...
          draw_marx_text(TUX_BORED, instructions, 1);
          ...
          else if (event.type == SDL_MOUSEBUTTONDOWN && valid_click(event.button.button))
            {
              if (event.button.x >= r_ttools.w && event.button.x < WINDOW_WIDTH - r_ttoolopt.w &&
                       ...
                       event.button.y < (button_h * buttons_tall + r_ttools.h))
                {
                  ...
                  draw_marx_text(TUX_BORED, freeme, 1);
                  free(freeme);
...
/**
 * Play an animated slideshow within Marx Paint.
...
static void do_render_cur_text(int do_blit)
{
  ...
  if (cursor_y > ((button_h * buttons_tall + r_ttools.h) - MarxPaint_Font_FontHeight(getfonthandle(cur_font))))
    {
      cursor_y = ((button_h * buttons_tall + r_ttools.h) - MarxPaint_Font_FontHeight(getfonthandle(cur_font)));
    }
      ...
      if (cur_label != LABEL_SELECT)
        {
          ...
          update_canvas_ex_r(old_cursor_x - 1,
                             old_cursor_y - 1,
                             old_cursor_x + 1, old_cursor_y + 1 + MarxPaint_Font_FontHeight(getfonthandle(cur_font)), 0);
  ...
  if (!do_blit)
    {
      ...
      /*            cursor_y - 1, */
      /*            cursor_x + 1 +  MarxPaint_Font_FontHeight(getfonthandle(cur_font)) * 3, */
      /*            cursor_y + 1 + MarxPaint_Font_FontHeight(getfonthandle(cur_font)), 0); */
...
static SDL_Surface *load_svg(const char *file)
{
  ...
  /* We want to render at full Marx Paint canvas size, so that the stamp
...
static SDL_Surface *load_svg(const char *file)
{
  ...
  /* Pick best scale to render to (for the canvas in this instance of Marx Paint) */
  ...
  for (plc = 0; plc < NUM_MAGIC_PLACES; plc++)
    {
      if (plc == MAGIC_PLACE_GLOBAL)
        ...
        place = strdup("/Library/Application Support/MarxPaint/plugins/");
#endif
      else
        continue;               /* Huh? */
      ...
      if (d != NULL)
        {
          ...
          if (plc == MAGIC_PLACE_GLOBAL)
            ...
            magic_api_struct->data_directory = strdup("/Library/Application Support/MarxPaint/plugins/data");
#endif
          else
                      ...
                      if (magic_handle[num_plugin_files] != NULL)
                        {
                          ...
                          if (magic_funcs[num_plugin_files].api_version == NULL)
                            ...
                            {
                              fprintf(stderr,
                                      "Warning: plugin %s uses Marx Paint 'Magic' tool API version %x,\nbut Marx Paint needs version %x.\n",
...
static int do_new_dialog(void)
{
                      ...
                      if (img != NULL)
                      ...
                      else
                        {
                          ...
                          if (d_places[num_files] == PLACE_PERSONAL_TEMPLATES_DIR ||
                            ...
                            {
                              /* Make sure we have a ~/.marxpaint/[starters|templates] directory: */
  ...
  draw_marx_text(TUX_BORED, tool_tips[TOOL_NEW], 1);
  ...
  if (cur_tool == TOOL_MAGIC)
    {
      ...
      /* In case the Magic tool's switchin() called update_progress_bar(),
         let's put the old Marx text back: */
...
static void setup_config(char *argv[])
{
  ...
  /* Mac OS X: Use a "marxpaint.cfg" file in the current folder */
  struct cfginfo tmpcfg_curdir;

  memset(&tmpcfg_curdir, '\0', sizeof tmpcfg_curdir);
  parse_file_options(&tmpcfg_curdir, "./marxpaint.cfg");
  ...
  if (tmpcfg_cmd.savedir)
    ...
    {
#ifdef _WIN32
      savedir = GetDefaultSaveDir("MarxPaint");
#elif __BEOS__
      savedir = strdup("./marxpaint");
#elif __HAIKU__
      ...
      asprintf((char **)&savedir, "%s/%s", buffer, "MarxPaint");
#elif __APPLE__
      savedir = strdup(apple_preferencesPath());
#else
      int tmp;

      tmp = asprintf((char **)&savedir, "%s/%s", home, ".marxpaint");
      if (tmp < 0)
        {
...
#endif
      safe_snprintf(str, sizeof(str), "%s/MarxPaint", picturesdir);
      free(picturesdir);
  ...
  safe_snprintf(str, sizeof(str), "%s/marxpaint.cfg", savedir);       /* FIXME */
#elif defined(__BEOS__) || defined(__HAIKU__)
  /* BeOS: Use a "marxpaint.cfg" file: */
  strcpy(str, "marxpaint.cfg"); /* safe; sufficient size */
#elif defined(__APPLE__)
  /* macOS, iOS: Use a "marxpaint.cfg" file in the Marx Paint application support folder */
  safe_snprintf(str, sizeof(str), "%s/marxpaint.cfg", apple_preferencesPath());

#else
  /* Linux and other Unixes:  Use 'rc' style (~/.marxpaintrc) */
  /* it should it be "~/.marxpaint/marxpaintrc" instead, but too late now */
  safe_snprintf(str, sizeof(str), "%s/.marxpaintrc", home);
#endif
  ...
  if (tmpcfg_usr.parsertmp_sysconfig != PARSE_NO)
    {
      ...
      parse_file_options(&tmpcfg_sys, "marxpaint.cfg");
#elif defined(__APPLE__)
      /* EP added this conditional section for Mac to fix
         folder & extension inconsistency with Marx Paint Config application) */
      /* macOS, iOS: Use a "marxpaint.cfg" file in the *global* Marx Paint
         application support folder */
      safe_snprintf(str, sizeof(str), "%s/marxpaint.cfg", apple_globalPreferencesPath());
      parse_file_options(&tmpcfg_sys, str);
#else
      /* normally /etc/marxpaint/marxpaint.conf */
      parse_file_options(&tmpcfg_sys, CONFDIR "marxpaint.conf");
#endif
  ...
  if (argv0)
    {
      ...
      // E.g., "/Applications/MarxPaint.app/Contents/MacOS/marxpaint"
          ...
          if (time_now < time_lock + 30)
            {
              ...
              printf
                ("You have already started marxpaint less than 30 seconds ago.\n"
                 "To prevent multiple executions by mistake, MarxPaint will not run\n"
                 "before 30 seconds have elapsed since it was last started.\n"
                 "\n" "You can also use the --nolockfile argument, see marxpaint(1).\n\n");
...
static void setup(void)
{
...
#ifndef WIN32
  putenv((char *)"SDL_VIDEO_X11_WMCLASS=MarxPaint.MarxPaint");
#endif
  ...
  seticon();
#endif
  SDL_WM_SetCaption("Marx Paint", "Marx Paint");
  ...
  if (big_title)
    img_title_marxpaint = loadimage(DATA_PREFIX "images/title-marxpaint-2x.png");
  else
    img_title_marxpaint = loadimage(DATA_PREFIX "images/title-marxpaint.png");

  SDL_FillRect(screen, NULL, SDL_MapRGB(screen->format, 255, 255, 255));

  dest.x = ((WINDOW_WIDTH - img_title->w - (img_title_marxpaint->w / 2)) / 2) + (img_title_marxpaint->w / 2) + 20;
  ...
  dest.x = 10;
  if (big_title)
    dest.y = WINDOW_HEIGHT - img_title_marxpaint->h - img_progress->h - 40;
  else
    dest.y = (WINDOW_HEIGHT - img_title->h) + img_title_marxpaint->h * 0.8 + 7;

  SDL_BlitSurface(img_title_marxpaint, NULL, screen, &dest);
  ...
  medium_font = MarxPaint_Font_OpenFont(PANGO_DEFAULT_FONT,
  ...
  large_font = MarxPaint_Font_OpenFont(PANGO_DEFAULT_FONT,
  ...
  small_font = MarxPaint_Font_OpenFont(PANGO_DEFAULT_FONT, DATA_PREFIX "fonts/default_font.ttf",
#ifdef __APPLE__
  ...
  /* Load tip marx images: */
  for (i = 0; i < NUM_TIP_TUX; i++)
    img_marx[i] = loadimagerb(marx_img_fnames[i]);
  ...
  for (i = 0; i < NUM_TITLES; i++)
    {
      if (strlen(title_names[i]) > 0)
        {
          MarxPaint_Font *myfont = large_font;
...
static void claim_to_be_ready(void)
{
  ...
  SDL_FreeSurface(img_title_marxpaint);
...
int main(int argc, char *argv[])
{
  ...
  freopen("/tmp/marxpaint.log", "w", stdout);    /* redirect stdout to a file */
 ...
 *   (note: currently, Marx Paint only puts things in the PICTURES one)
...
static int export_gif(int *selected, int num_selected, char *dirname, char **d_names, char **d_exts, int speed) {
  ...
  /* For now, always saving GIF using the size of Marx Paint's window,
...
/**
 * Copy an image (just the main PNG) from Marx Paint's "saved"
...
static int export_pict(char * fname) {
  ...
  if (fi == NULL)
    {
      fprintf(stderr, "Cannot export from saved Marx Paint file '%s'\nThe error that occurred was:\n%s\n\n", fname, strerror(errno));
      return FALSE;
    }
 ...
 * export directory (e.g., ~/Pictures/MarxPaint, or whatever "--exportdir" says).
 *
 ...
 *   (e.g., /home/username/Pictures/MarxPaint/2020072620110100.gif")
