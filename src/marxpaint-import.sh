...
# "Marx Paint Import"
# Import an arbitrary GIF, JPEG or PNG into Marx Paint
...
if [ $# -ne 0 ]; then
  if [ $1 = "--help" ]; then
    # --help, show usage:
    echo
    echo "marxpaint-import"
    echo
    echo "Imports an arbitrary image (GIF, JPEG, PNG, etc. format)"
    echo "into Marx Paint (see: marxpaint(1)) so that it appears in the"
    echo "'Open' dialog."
    echo 
    echo "Usage: marxpaint-import filename(s)"
    echo "       marxpaint-import --help"
    echo
    exit
  fi
fi
...
x=`grep -m 1 "^savedir=" /usr/local/etc/marxpaint/marxpaint.conf`
if test $? = 0 ; then
  SAVEDIR=`echo $x | cut -d = -f 2-99`
fi

# First, check /etc/
x=`grep -m 1 "^savedir=" /etc/marxpaint/marxpaint.conf`
if test $? = 0 ; then
  SAVEDIR=`echo $x | cut -d = -f 2-99`
fi

# First, check $HOME
x=`grep -m 1 "^savedir=" $HOME/.marxpaintrc`
if test $? = 0 ; then
  SAVEDIR=`echo $x | cut -d = -f 2-99`
fi
...
# Determine appropriate size for images, based on Marx Paint's current
# configuration

# First, assume 800x600 Marx Paint
window_width=800
window_height=600

# First, check /usr/local/etc/
x=`grep -m 1 "^windowsize=" /usr/local/etc/marxpaint/marxpaint.conf`
if test $? = 0 ; then
...
# First, check /etc/
x=`grep -m 1 "^windowsize=" /etc/marxpaint/marxpaint.conf`
if test $? = 0 ; then
  window_width=`echo $x | cut -d = -f 2 | cut -d x -f 1`
  window_height=`echo $x | cut -d = -f 2 | cut -d x -f 2`
fi
...
# First, check $HOME
x=`grep -m 1 "^windowsize=" $HOME/.marxpaintrc`
if test $? = 0 ; then
  window_width=`echo $x | cut -d = -f 2 | cut -d x -f 1`
  window_height=`echo $x | cut -d = -f 2 | cut -d x -f 2`
fi
...
echo "Using $width x $height images (for $window_width x $window_height Marx Paint"
