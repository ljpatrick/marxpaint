...
什麼是 'Marx Paint?'
       ...
       "Marx Paint" 是專為年幼的兒童(三歲及以上)設計的繪圖程式，他提供容易使用的介面，
       有趣的音效，及鼓勵並帶領兒童使用這個軟體的卡通造型的吉祥物。 他提供一張空白的圖紙和許多的繪圖工具，可以幫助你的小孩啟發創造力。

授權：

       Marx Paint 是一個開放源碼的專案，以 GNU 通用公共授權(GPL)釋出的自由軟體。 他是自由的，而且也可以取得程式的源碼
...
目標：
       ...
       容易和有趣
               Marx Paint 是一個給年幼兒童用的簡易繪圖程式。他並不是一個一般用途的繪圖程式，
               他特點是在於有趣和容易使用。音效和卡通人物協助使用者了解現在要作什麼， 也讓他們保持歡樂。還有一個超大的卡通造型的滑鼠游標。

       擴充性
               Marx Paint 是可擴充的。筆刷及橡皮圖章的形狀可以加上及抽掉的。
               例如老師可以加上一組動物的圖章，讓他的學生們來畫動物的生態。 每一個形狀可以在被畫的時候發出他們專屬的聲音，
               也可以在小朋友選擇一個形狀時顯示出文字的說明。

       移植性
               Marx Paint 移植到許多不同的平台上，像是 Windows, Macintosh, Linux 等，
               他們的使用介面看起來都是一樣的。Marx Paint 可以在舊的機器上跑的不錯 (像是 Pentium
...
載入 Marx Paint
  ...
  Linux/Unix 的使用者
         ...
         Marx Paint 會建立一個啟動的圖示在你的 GNOME 或 KDE 的"圖形"目錄選單之中。
  ...
  Windows 的使用者
         ...
         如果你是使用安裝程式安裝 Marx Paint 的話，在安裝過程中會詢問你要將捷徑放在開始功能表的哪裡，
         還有桌面的捷徑。如果你同意建立了捷徑，你就可以直接由開始功能表中的 'Marx Paint' 目錄裡啟動 Marx Paint (如：在
         Windows XP 的"所有程式"裡)，或是雙擊桌面上 "Marx Paint" 的圖示。

         如果你是下載安裝 Marx Paint 的 ZIP 壓縮檔，或是你是由安裝程式安裝的，但選擇不建立捷徑的話， 你必須要到 'Marx
         Paint' 的目錄中雙擊 marxpaint.exe 的圖示。

         'Marx Paint' 的安裝程式預設會將 Marx Paint 的目錄放在 "C:\Program Files\>/code>" 裡，
         你可以在安裝程式執行過程裡來改變他。

         如果你使用下載的 ZIP 壓縮檔，就看你解開壓縮檔時要將 Marx Paint 的目錄放到哪裡去。
...
主畫面
       ...
       中間：圖紙
               ...
               備註: 圖紙的尺寸就是 Marx Paint 的大小。你可以使用 Marx Paint Config
               設定工具或是其他的方式來變更 Marx Paint 的大小。詳細內容請見 OPTIONS 文件。
       ...
       底部：說明區域
               ...
               當你在畫畫時，在螢幕的最底部，Linux 的企鵝 - Marx 會提供提示和訊息。
  ...
  繪圖工具
     ...
     ----------------------------------------------------------------------
         ...
         圖章 (橡皮圖章)
                 ...
                 不同的圖章可以有不同的音效和講解，在左下角的按鈕(靠近 Linux 的企鵝 Marx 的地方)可以讓你
                 重覆目前圖章的音效和講解。

                 (註：如果設定了 "nostampcontrols" 選項的話，Marx Paint
         ...
         字母
                 ...
                 當一行輸入到一半時滑鼠在其他圖面上點一下，整行文字就會移過去。 而你可以繼續輸入。
                      ...
                      各國文字輸入
                              ...
                              Marx Paint 可以輸入不同語言的文字。大部份的拉丁文 (A-Z, ñ, è, 等等.)
                              可以直接輸入。 部份語言輸入前需要將 Marx Paint
                              切換到不同的輸入模式，而且部份文字必須使用組合鍵來輸入。

                              當 Marx Paint 的地區設定為某一種語言時，就可以提供不同的輸入模式，
  ...
  其他的控制
         ...
         列印
                 ...
                 再點擊「列印」鈕來呼叫出列印的交談框。 不過如果你是執行全螢幕模式的話就不能這麼作。請見後面說明。
                      ...
                      關閉列印功能
                              ...
                              如果不要列印的選項被設定的話 (可以在 Marx Paint 的規劃檔裡設定
                      ...
                      限制列印功能
                              ...
                              如果延遲列印的選項被設定的話 (可以在 Marx Paint 的規劃檔裡設定
                      ...
                      列印命令
                              ...
                              Marx Paint 的列印是將圖檔轉換成 PostScript
                              格式之後再將他傳給外部的程式。預設的程式是：

                                lpr

                              這組命令可以在 Marx Paint 的規劃檔裡用 "printcommand"
                              的設定來改變他。
                              ...
                              這組命令可以在 Marx Paint 的規劃檔裡用 "altprintcommand"
                              的設定來改變他。
                      ...
                      印表機設定
                              ...
                              預設的情況下，按下「列印」按鈕後 Marx Paint 直接用預設的印表機設定值，
                              將圖畫由預設印機機列印出來。
                              ...
                              你可以使用 "--printcfg" 啟動參數，或是在 Marx Paint 的規劃檔裡設定
                              "printcfg=yes" 的選項來儲存 Marx Paint 的印表機規劃檔
                              ("marxpaint.cfg")。
     ...
     ----------------------------------------------------------------------
         ...
         離開
                 ...
                 按下「離開」按鈕或是 [Escape] 按鍵就會關閉 Marx Paint 。
                 ...
                 註：當你將圖畫儲存之後，下一次你再開啟 Marx Paint 的時候，預設就會開啟這張圖。

                 註:「離開」按鈕和 [Escape] 鍵可以被關閉。(例如在 Marx Paint 設定程式 裡選取
                 "Disable 'Quit' Button" ，或是在啟動 Marx Paint 時使用 "--noquit"
                 命令列參數)

                 在這個情況下可以使用 Marx Paint 視窗右上方「關閉視窗」的叉叉按鈕(如果不是在全螢幕模式)， 或是使用
                 [Alt] + [F4] 組合鍵或許可以離開。
         ...
         靜音
                 ...
                 請注意當音效是被完全關閉時(例如在Marx Paint Config 設定工具裡沒有開啟音效，或是使用
                 "--nosound"
                 命令列參數來啟動Marx Paint)，用[Alt] + [S]組合鍵是沒有效果的。(當家長或老師希望關閉音效時，這組合鍵就不能用來開啟音效了。)

     ----------------------------------------------------------------------

                               載入其他的圖畫到 Marx Paint

     當使用 Marx Paint 的開啟圖畫功能時，只會顯示出你用 Marx Paint 所畫的圖， 如果你想載入其他的圖形或照片到 Marx Paint
     裡來編輯的話該怎麼作呢？

     要這麼作的話，你只需要將圖形轉成 PNG 的格式，然後把這圖形放到 Marx Paint 的儲存目錄 ：

     Windows Vista
             放到使用者的 "AppData" 目錄：
             例如："C:\Users\(user name)\AppData\Roaming\MarxPaint\saved\"
     ...
     Linux/Unix
             放到一個隱藏的 ".marxpaint" 目錄，這個目錄位於使用者的家目錄： "$(HOME)/.marxpaint/saved/"

     備註: 你可以從這個目錄來複製圖畫，或是用其他的程式來開啟圖畫。

  使用 'marxpaint-import'

       Linux 和 Unix 的使用者可以使用 "marxpaint-import" 這支 shell script ，他使用一些 NetPBM
       的工具來轉換圖檔格式 ("anytopnm")， 改變圖形的大小使其符合 Marx Paint 的圖紙 ("pnmscale")，
       再將圖檔轉換成 PNG 格式 ("pnmtopng")。

       這也會用 "date" 指令來取得目前的時間和日期， 作為 Marx Paint 儲存時的檔名之用。(記得嗎，當你在儲存或是開啟圖畫時，
       都不需要知道圖檔的名稱的。)

       使用 'marxpaint-import' 時只要在命令列打上這個指令， 並且給他你想要轉換的檔案的檔名。

       這樣就會將圖檔轉好並放在 Marx Paint 的儲存目錄中。 (註：如果你想要幫其他的使用者轉換圖檔的話，例如你的小孩，
       ...
       範例的第一行 ("marxpaint-import grandma.jpg") 是執行的指令， 接下來的二行是程式執行時的輸出訊息。

       現在你可以載入 Marx Paint ，在「開啟」的一堆縮圖中可以看到轉換後的圖形， 雙擊這個縮圖就可以開啟。
...
手工處理
       ...
       當 Marx Paint 開啟一張與圖紙尺寸不符合的圖檔時，他會縮放(有時會裁去 邊緣)圖畫以符合圖紙。

       要避免圖檔被拉伸或裁邊，你可以重新設定 Marx Paint 圖紙的尺寸， 這個尺寸就視 Marx Paint
       視窗的大小。如果是全螢幕模式的話， 就必須調整解析度。(注意: 預設的解析度是 800x600.) 詳見「計算圖檔的尺寸」

       用 PNG 格式來儲存圖形，強烈的建議你，圖檔的名稱使用目前的日期及時間， Marx Paint 使用的規則是：
       ...
       將這個 PNG 檔放到你 Marx Paint 的儲存目錄中。 (請參閱前面的說明。)

  計算圖檔的尺寸

         Marx Paint 的圖紙寬度就是視窗的寬度 (例如 640, 800 或是 1024 像素) 減去 192。

         計算 Marx Paint 的圖紙高度需要幾個步驟:
         ...
         範例: Marx Paint 執行全螢幕模式於 1440x900.
           ...
           * 圖紙高度的計算如下:
               ...
               4. 15 * 48 等於 720
               5. 720 + 40 等於 760
         所以當視窗是 1440x900 時 Marx Paint 的圖紙是 1248x760.

     ----------------------------------------------------------------------

                                      其他文件

       Marx Paint 其他的文件包括：(在 "docs" 目錄裡)
         * 魔術工具文件
           每一個目前所安裝的魔術工具的文件
         * AUTHORS.txt
           作者及貢獻者的名單
         ...
         * INSTALL.txt
           介紹如何編譯及安裝
         * EXTENDING.html
           詳細的說明如何建立筆刷，圖章和起始圖案，以及新增字型來擴充 Marxpaint。
         * OPTIONS.html
           詳細介紹命令列及規劃檔的選項，給不想或不能用 Marx Paint Config 的人
         * PNG.txt
           說明如何建立給 Marx Paint 使用的 PNG 格式的圖檔
         * SVG.txt
           說明如何建立給 Marx Paint 使用的 SVG 格式的向量圖
